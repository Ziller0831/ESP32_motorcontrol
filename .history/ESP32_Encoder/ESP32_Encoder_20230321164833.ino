...
void cmd_velocity_receiv(void *pvParam, const geometry_msgs::Twist &cmdVel){
    while(1){
        ...
        if (cmdVel.angular.z != 0.0){
            ...
            double avgDiff = (PrevDiff + PrevprevDiff + currDiff) / 3;
...
void Motor_control(void *pvParam){
    ...
    While(1){
        if ((Left_PWM_Req * Left_MT_Vel < 0 && L_PWM_out != 0) ||
            (Right_PWM_Req * Right_MT_Vel < 0 && P_PWM_out != 0)){
            Left_PWM_Req = 0;
            Right_PWM_Req = 0;
        }
...
void setup(){
    ...
    xTaskCreatePinnedToCore(SimpleMotor_control, "Task:Motor_control", 1024, NULL, 3, &Task_Motorcontrol);
